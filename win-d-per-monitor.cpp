...
When you press win+d, it only minimizes/restores the windows on the monitor where the mouse cursor is located.
...
#define MIN_SIZE_WND (100 * 100)
        ...
        auto enumMonitorWnd = [&](HWND hDesktop, HMONITOR hMonitor)
        {
            ...
            while (hWndCcc)
            {
                ...
                // 处理需要忽略的类名.
                {
                    static std::set<std::string> ignoredClass = {"Shell_SecondaryTrayWnd", "Shell_TrayWnd", "WorkerW",
                                                                 "SysShadow", "TaskListThumbnailWnd"};
